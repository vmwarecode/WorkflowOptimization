...
class VxRailJsonConverterPatch:
    ...
    def do_patching(self, converter, is_primary):
        ...
        if select_option.lower() == 'yes' or select_option.lower() == 'y':
            ...
            if pg_types_to_vmnics is not None:
                vmnics_list = []
    ...
    def create_vmnics_spec_for_system_dvs_advanced_profile(self, dvs_vmnics_mapping, dvs_name):
        vmnics = []
        for vmnic in dvs_vmnics_mapping:
            vmnics.append({'id': vmnic, 'vdsName': dvs_name})
        return vmnics
